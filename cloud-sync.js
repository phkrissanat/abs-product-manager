...
(function () {
  ...
  function showLoginModal(prefillEmail, errorMsg) {
    ...
    modalEl.innerHTML = `
      <div style="width:min(340px,90vw);background:#030f1e;border:1px solid rgba(0,229,255,.4);
        ...
        <div style="color:rgba(180,240,255,.7);font-size:13px;margin-bottom:16px;line-height:1.5">
          ใช้อีเมล/รหัสผ่านเดียวกับที่ตั้งไว้ตอนแรก เพื่อให้ข้อมูล sync กันทุกเครื่อง
        </div>
        <form id="absLoginForm" autocomplete="on">
          <input id="absLoginEmail" name="email" type="email" autocomplete="username" placeholder="อีเมล" value="${(prefillEmail || '').replace(/"/g, '&quot;')}"
          ...
          <input id="absLoginPassword" name="password" type="password" autocomplete="current-password" placeholder="รหัสผ่าน (อย่างน้อย 6 ตัวอักษร)"
            style="width:100%;padding:10px;margin-bottom:10px;background:rgba(0,229,255,.05);
            border:1px solid rgba(0,229,255,.3);border-radius:4px;color:#cdf4ff;font-size:14px" />
          <label style="display:flex;align-items:center;gap:8px;margin-bottom:14px;color:rgba(180,240,255,.8);font-size:13px;cursor:pointer">
            <input id="absLoginRemember" type="checkbox" checked style="width:16px;height:16px;accent-color:#00e5ff;cursor:pointer" />
            จดจำการเข้าสู่ระบบในเครื่องนี้ (ปิดถ้าใช้เครื่องคนอื่น/สาธารณะ)
          </label>
          ${errorMsg ? `<div style="color:#ff6688;font-size:12px;margin-bottom:10px">${errorMsg}</div>` : ''}
          <button id="absLoginSubmit" type="submit" style="width:100%;padding:10px;background:linear-gradient(135deg,rgba(0,229,255,.2),rgba(0,100,160,.4));
    ...
    const rememberInput = modalEl.querySelector('#absLoginRemember');
    const formEl = modalEl.querySelector('#absLoginForm');
    const submit = () => {
      ...
      attemptSignIn(email, password, remember);
    };
    formEl.addEventListener('submit', (e) => { e.preventDefault(); submit(); });
  }

  function attemptSignIn(email, password, remember) {
    setStatus('กำลังเข้าสู่ระบบ...', false);
    // LOCAL persistence = stays signed in after closing the browser (remembered).
    // SESSION persistence = signed out as soon as the tab/browser closes — nothing
    // left behind on a shared or public computer.
    const persistence = remember
      ? firebase.auth.Auth.Persistence.LOCAL
      : firebase.auth.Auth.Persistence.SESSION;
    auth.setPersistence(persistence)
      .then(() => auth.signInWithEmailAndPassword(email, password))
      .then((cred) => onAuthSuccess(cred, email, password, remember))
      .catch((err) => {
        // No auto-create-account fallback here on purpose: the real account
        // already exists, so any failed sign-in (wrong password, unknown
        // email, etc.) should show an error — not silently create a new,
        // empty, unrelated account. That was the earlier bug: it made the
        // login screen accept literally anything.
        showLoginModal(email, 'อีเมลหรือรหัสผ่านไม่ถูกต้อง');
      });
  }

  function onAuthSuccess(cred, email, password, remember) {
    removeLoginModal();
    docRef = db.collection('abs_sync').doc(cred.user.uid);
    attachListener();
    setStatus('ซิงค์แล้ว', true);
    if (!remember) return; // shared/public computer — leave no trace on this device
    storeCreds(email, password);
    // Offer to save into Chrome / Google Password Manager (in addition to
    // the app's own localStorage auto-login) — best-effort, ignore if the
    // browser doesn't support the Credential Management API.
    if (window.PasswordCredential) {
      try {
        const pc = new PasswordCredential({ id: email, password, name: 'ABS Product Manager' });
        navigator.credentials.store(pc).catch(() => {});
      } catch (e) { /* ignore */ }
    }
  }
  ...
  function attachListener() {
    docRef.onSnapshot((snap) => {
      if (!snap.exists) {
        setStatus('กำลังส่งข้อมูลขึ้นคลาวด์ครั้งแรก...', true);
        if (typeof products !== 'undefined' && products.length && !applyingRemote) push(products);
        return;
      }
  ...
  if (stored.email && stored.password) {
    setStatus('กำลังเข้าสู่ระบบ...', false);
    attemptSignIn(stored.email, stored.password, true);
  } else {
    showLoginModal();
  }
