  ...
  function attemptSignIn(email, password) {
    ...
    auth.signInWithEmailAndPassword(email, password)
      ...
      .catch((err) => {
        ...
        // returns 'auth/invalid-credential' instead of 'auth/user-not-found'
        // even when no account exists yet — so try both as "account missing".
        if (err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential') {
          auth.createUserWithEmailAndPassword(email, password)
            .then((cred) => onAuthSuccess(cred, email, password))
            .catch((err2) => {
              if (err2.code === 'auth/email-already-in-use') {
                // Account really does exist — the original error was a genuinely wrong password.
                showLoginModal(email, 'รหัสผ่านไม่ถูกต้อง ลองใหม่อีกครั้ง');
              } else {
                showLoginModal(email, 'สร้างบัญชีไม่สำเร็จ: ' + err2.message);
              }
            });
        } else {
